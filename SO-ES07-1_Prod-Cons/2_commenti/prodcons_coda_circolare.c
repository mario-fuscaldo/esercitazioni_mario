...
int main() {
	...
	if(ds_sem<0) { perror("SEM errore"); exit(1); }
        ...
        semctl(ds_sem, SPAZIO_DISPONIBILE, SETVAL, DIM_BUFFER); //all'inizio è tutto libero quindi il semaforo va messo alla dimensione del buffer
        semctl(ds_sem, MESSAGGIO_DISPONIBILE, SETVAL, 0);
	
        //SEMAFORI COMPETIZIONE tra i prod e i cons
        semctl(ds_sem, MUTEX_C, SETVAL, 1);
        semctl(ds_sem, MUTEX_P, SETVAL, 1);		//devono stare a 1, se stanno a 0 nessuno accederà mai
