...
void produttore(struct prodcons * p, int ds_sem) {
    ...
    //cerca la prima posizione vuota
    while(indice <= DIM_BUFFER && p->stato[indice] != BUFFER_VUOTO) {  //trova la prima quando la condizione è falsa e quindi esce
        indice++;                                                      //perciò nella condizione c'è != buffer vuoto
    }
