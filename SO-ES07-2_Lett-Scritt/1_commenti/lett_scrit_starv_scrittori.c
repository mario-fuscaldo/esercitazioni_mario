...
int main(){
        ...
        pid_t pid;              //può anche essere int
        Buffer *buf;

        numlettori = 6;         //deciso a priori
        numscrittori = 6;
        ...
        semctl(sem, MUTEXL, SETVAL, 1);         //i lettori all'inizio possono leggere
        ...
        for (k=0; k<num_processi; k++) {
                ...
                if (pid == 0)  {                //processo figlio, decidiamo lettori e scrittori tramite pari e dispari
                        if ( (k%2) == 0) {
